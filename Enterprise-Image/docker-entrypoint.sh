...
if [[ "$REPLICATION_ENABLED" == "true" ]] ; then
        echo "$REPLICATION_KEY" > /data/configdb/replkey
        chown mongodb:mongodb /data/configdb/replkey
        chmod 0600 /data/configdb/replkey
fi
if [[ -z "$ENCRYPTION_KEY" ]]; then
        echo "ENCRYPTION_KEY not defined"
else       
        echo "$ENCRYPTION_KEY" > /data/configdb/encryptionkey
        chown mongodb:mongodb /data/configdb/encryptionkey
        chmod 0600 /data/configdb/encryptionkey
fi
originalArgOne="$1"
...
if [ "$originalArgOne" = 'mongod' ]; then
	...
	if [ -n "$shouldPerformInitdb" ]; then
		...
		if [ "$MONGO_INITDB_ROOT_USERNAME" ] && [ "$MONGO_INITDB_ROOT_PASSWORD" ]; then
			...
			EOJS
                        if [[ -z "${MONGODB_USER}" ]] || [[ -z "${MONGODB_PASSWORD}" ]] || [[ -z ${DB_NAME} ]] ; then
                            echo "MONGODB_USER or MONGODB_PASSWORD or DB_NAME not defined"
                        else
                            "${mongo[@]}" "$rootAuthDatabase" -u "$MONGO_INITDB_ROOT_USERNAME" --password "$MONGO_INITDB_ROOT_PASSWORD" <<-ET
                                    db.createUser({
